...
export const indicated = functions.firestore.onDocumentCreated(
  {document: 'referrals/{referralsId}', secrets: [EMAIL_USER, EMAIL_PASS, WEBHOOK_BOTCONVERSA]},
  async event => {
    // Pegando os dados da indicação
    const newReferral = event.data?.data();
        ...
        <div class='container'>
          <div style="font-family: familjen grotesk;" class="div">
            ...
            <div class='indication'>
              <span>Indicador: ${newReferral.indicator_name}</span>
              <span>Nome do Indicado: ${newReferral.name}</span>
              <span>Telefone do Indicado: ${newReferral.phone}</span>
              <span>Produto desejado: ${newReferral.product}</span>
              <span>Observações: ${newReferral.observations}</span>
            </div>
            <p></p>
    ...
    const payload = {
      indicator_name: newReferral.indicator_name,
      indication_phone: newReferral.phone,
      indication_name: newReferral.name,
      indication_product: newReferral.product,
      indication_observations: newReferral.observations,
      unit_name: unitName,
